...
const User = require("../models/user"); // Required for GreenCoins wallet
...
router.get("/requests", verifyToken, async (req, res) => {
  try {
    ...
    const transformedRequests = requests.map((request) => ({
      ...request,
      requester: request.requester ? { name: request.requester.username, email: request.requester.email } : { name: "Unknown", email: "N/A" },
    }));
...
router.post("/offer", verifyToken, async (req, res) => {
  try {
    ...
    // 🟢 Gamification: Reward Driver 2 GreenCoins for offering a ride
    try {
