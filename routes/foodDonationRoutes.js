...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    ...
    let folderName = "greenverse_food";

    if (file.fieldname === "foodImage") {
      folderName = "greenverse_food/donor_uploads";
    }

    if (file.fieldname === "receiverImage") {
      folderName =
        "greenverse_food/receiver_confirmations";
    }
    ...
    try {
      ...
      const user =
        await User.findById(req.userId).select(
          "username email profilePic role"
        );
    ...
    try {
      ...
      res.json({
        ...
        summary: {
          ...
          totalFoodReceivedKg: Number(
            totalFoodReceivedKg.toFixed(2)
          ),

          totalCarbonImpact: Number(
            totalCarbonImpact.toFixed(2)
          ),
        },
      ...
      console.error(
        "Get Received Food Error:",
        error
      );

      res.status(500).json({
        message:
          "Failed to fetch received food",
      });
    }
  }
);
    ...
    try {
      ...
      try {
        ...
        await User.findByIdAndUpdate(
          ...
          {
            $inc: {
              greenCoins: coinsEarned,
            },
          }
        );

        console.log(
          `🎉 Awarded ${coinsEarned} GreenCoins`
        );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      donation.receiverImage =
        req.file?.path || "";

      donation.receiptConfirmed = true;
